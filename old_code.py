    ...
    def process_data(self, data, **kwargs):
        try:
            ...
            for detected_object in copy.deepcopy(data["detections"]):
                ...
                # Create a Polygon object using the extracted coordinates
                ...
                if detected_object["name"] == object_class_name and detected_object["confidence"] >= 0.8:
                    ...
                    for _id in loaded_camera_ids[source_details["source_id"]]["indexes"]:
                        ...
                        #     logger.debug(polygons[_id])
                        # if detected_polygon.bounds in polygons[_id].bounds:
                        # if Point(x_coordinate, y_coordinate).within(polygons[_id]) or detected_polygon.bounds in polygons[_id].bounds:

                        if polygons[_id].contains(detected_polygon):
...
#         self.scheduler = BackgroundScheduler(timezone=utc)
#         self.scheduler.add_job(partial(self.clear_cache, self.object_tracker), 'interval', seconds=10)
#         self.scheduler.start()

#     def clear_cache(self, sample_generator):
#         utc_now = datetime.datetime.utcnow()
#         five_minutes_ago = datetime.timedelta(seconds=30)
